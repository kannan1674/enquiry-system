...
export function AdminAuthGuard({ children }: AdminAuthGuardProps) {
  ...
  useEffect(() => {
    const checkAuthorization = () => {
      ...
      console.log('🔐 [AdminAuthGuard] Authorization check:', {
        isAuthenticated,
        roleId,
        userRole,
        authLoading
      ...
      // Check if user is authenticated and has admin role (roleId = 1)
      if (isAuthenticated && userRole === 1) {
        console.log('✅ [AdminAuthGuard] User is authorized as admin');
        setIsAuthorized(true);
        setIsChecking(false);
      } else if (isAuthenticated && userRole === 2) {
        console.log('❌ [AdminAuthGuard] User is authenticated but not admin (roleId = 2)');
        showError('403 Unauthorized: You do not have permission to access this page');
        router.push('/Home');
        setIsChecking(false);
      } else if (!isAuthenticated) {
        console.log('❌ [AdminAuthGuard] User is not authenticated');
        showError('403 Unauthorized: Please log in to access this page');
        router.push('/Home');
        setIsChecking(false);
      } else {
